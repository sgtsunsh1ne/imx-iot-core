...
{
	int nSize;				/*!requested memory size */
	addr_t nPhyAddr;	/*!physical memory address allocated */
	addr_t nCpuAddr;	/*!cpu addr for system free usage */
	addr_t nVirtAddr;	/*!virtual user space address */
	VpuMemDescType nType;
	int nReserved[3];			/*reserved for future extension*/
}VpuMemDesc;
#endif
...
#else
	addr_t nInPhyInput;	//input buffer address
	addr_t nInVirtInput;
	int nInInputSize;
	addr_t nInPhyOutput;	//output frame address
	addr_t nInVirtOutput;
	unsigned int nInOutputBufLen;
#endif
