...
// Uncomment next line for message printing
//#define SDMA_DEBUG_PRINT(...)   DbgPrintEx(DPFLTR_IHVDRIVER_ID, 0xffffffff, __VA_ARGS__)
#define SDMA_DEBUG_PRINT(...)
...

{
    ...
    if (RegValue == CHN_PRI_DISABLED) {
        ...
        SDMA_DEBUG_PRINT("Channel %u start failed!\n", ChannelNumber);
        return STATUS_INVALID_DEVICE_STATE;
    }
...
#pragma prefast(suppress: 25024, "Channel 0 context is SDMA_CHANNEL0*")
    ...
    SdmaChannelConfigPtr = SdmaChannelPtr->ChannelConfigPtr;
#ifdef _ARM64_
    /* Clears SdmaChannelContextPtr manually because RtlZeroMemory causes 
    unaligned access. Address is not aligned for 64bit access (stp instruction).  */
    volatile ULONG * ptr = (ULONG *)SdmaChannelContextPtr;
    for (ULONG i = 0; i < (sizeof(SDMA_CHANNEL_CONTEXT) / sizeof(ULONG)); i++) {
        ptr[i] = 0;
    }
#else
...

{
    ...
    for (Index = 0; Index < ARRAYSIZE(SdmaRegsDump); ++Index) {
        ...
        SDMA_DEBUG_PRINT("%s = 0x%x\n\r", RegDumpPtr->NameSz, RegDumpPtr->Value);
    }
    SDMA_DEBUG_PRINT("%s end\n", __FUNCTION__);
    return;
}
